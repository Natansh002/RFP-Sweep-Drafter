...
export async function runSweep(tenantId, industryId, opts = {}) {
  const { width = 2, direct = false, detail = true, fetchImpl = fetch, now = new Date(), log = () => {}, source = "cli" } = opts;
  ...
  const pack = effectivePack(loadPack(industryId), tenant);
  ...
  const toRead = candidates.filter((c) => c.p.url).sort((a, b) => b.s.total - a.s.total).slice(0, limit);
  if (toRead.length) log(`  reading ${toRead.length} posting page(s)`);
  await pool(toRead, 6, async (c) => {
    const r = await get(c.p.url, fetchImpl, blocked);
  ...
  const library = loadData(`library/${tenant.id}.json`)?.entries ?? [];
  const byId = new Map();
  ...
  return {
    ...
    channelsRead: targets.length, postingsSeen, skipped,
