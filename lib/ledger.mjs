...
const MACHINE = ["industry", "industryStatus", "title", "buyer", "country", "url", "channel", "closeDate", "estimatedValue", "score", "band", "reasons", "flags", "suggestedAssignee", "keyDates", "requirements", "competitors", "incumbent", "libraryMatches", "sourceText", "capabilities", "team", "publishedDate", "noticeType", "sector"];

const STICKY = new Set(["sourceText", "closeDate", "estimatedValue", "keyDates", "requirements", "competitors", "incumbent", "libraryMatches", "buyer", "country"]);
...
export function ledgerPath(root, tenantId) {
  return path.join(root, "store", `${tenantId}.json`);
}
...
export function mergeRun(ledger, run) {
  ...
  for (const f of run.findings) {
    ...
    const changes = [];
    if (f.contentHash && cur.contentHash && f.contentHash !== cur.contentHash) changes.push("posting page content changed");
    ...
    if (changes.length) {
      ...
      cur.actions.push({ id: `addendum-${Date.now().toString(36)}`, title: `Review the change and acknowledge any addendum: ${changes.join("; ")}`, due: now.slice(0, 10), assignee: cur.assignee || "", done: false, source: "change-detection", rev: 0 });
    }
    if (f.contentHash) cur.contentHash = f.contentHash;
...
export function saveWorkspace(ledger, id, ws, { by } = {}) {
  const f = getFinding(ledger, id);
  const allowed = ["analysis", "answers", "proposal", "redTeam", "teamOverride", "documentName"];
