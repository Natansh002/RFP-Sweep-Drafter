...
export function profileLinks(homeUrl, facts, max = 7) {
  ...
  for (const l of facts.links ?? []) {
    ...
    if (/\.(pdf|jpe?g|png|gif|svg|webp|zip|docx?|xlsx?|mp4)$/i.test(u.pathname) || /\/(blog|news|events?|careers?|jobs|login|signin|sign-in|cart|support|knowledge-base|wp-content|wp-admin|tag|category|author|feed|sitemap|search|(privacy|terms|legal|cookie|contact|accessibility|disclaimer|gdpr)[\w-]*)(\/|$)/i.test(u.pathname)) continue;
