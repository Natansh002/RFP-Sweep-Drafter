...
import { contractorRole } from "./noticekind.mjs";
...
const STOP = new Set(("a an the and or of to for in on at by with from as is are be been being will can our your their its this that these those it we you they us not no " +
  "have has had said says say saying worked works working through issue issues also just like get got make made many much very really able use used using " +
  "into onto over about after before than then there here what when where which who whom whose why how all any each every both either neither other another such").split(" "));
...
const FILLER = new Set(("built build builds helps help helping manage manages managing run runs running replaces replace trust trusted trusts one place " +
  ...
  "funders funder love loved proud unlock empower empowers deliver delivers delivering make makes making designed purpose " +
  "excellent amazing awesome fantastic incredible outstanding wonderful happy pleased best-in-class world-class industry-leading cutting-edge " +
  "state-of-the-art next-generation leading-edge top-notch seamless seamlessly robust innovative").split(" "));
...
const GENERIC_WORD = new Set(("management system systems time data board boards organizations organization confidence business work need needs process processes tools tool " +
  ...
  "platforms program programs products product customers customer mobile technology decisions decision years year manual financial finance microsoft " +
  "reporting cloud digital online integrated connected powerful efficient efficiency easy simple better modern").split(" "));
...
export function profileLinks(homeUrl, facts, max = 7) {
  ...
  for (const l of facts.links ?? []) {
    ...
    if (/\.(pdf|jpe?g|png|gif|svg|webp|zip|docx?|xlsx?|mp4)$/i.test(u.pathname) || /\/(blog|news|events?|careers?|jobs|login|signin|sign-in|cart|support|knowledge-base|wp-content|wp-admin|tag|category|author|feed|sitemap|search|(privacy|terms|legal|cookie|contact|accessibility|aoda|wcag|disclaimer|gdpr|eula|licen[cs]e|copyright|trademarks?)[\w-]*)(\/|$)/i.test(u.pathname)) continue;
...
export function buildProfile({ website = null, pages = [], source = "website" } = {}) {
  ...
  const nameWords = new Set([...lower(name).split(/\W+/).filter((w) => w.length > 2), ...(host ? [host.split(".")[0]] : [])]);
  // A single word written with a capital in most places is a name (a testimonial's "Chris"), not an offering term.
  const rawText = pages.map((p) => p.text).join(" \n ");
  const properName = (w) => {
    const all = rawText.match(new RegExp(`(^|[^A-Za-z0-9])(${escRe(w)})(?![A-Za-z0-9])`, "gi")) ?? [];
    if (all.length < 2) return false;
    const capped = all.filter((m) => /^[^A-Za-z0-9]?[A-Z][a-z]/.test(m)).length;
    return capped / all.length >= 0.8;
  };
  const keywords = distinctiveTerms(pages.map((p) => p.text), headings)
    .filter((k) => !INDUSTRY_WORD.test(k) && !k.split(" ").some((w) => nameWords.has(w)))
    .filter((k) => k.includes(" ") || !properName(k))
 ...
 * What it sells (capabilities) 60, its own terms and product names 25, platforms 15: an RFP
 * whose title asks for something the company sells is a strong fit on that alone.
...
export function companyFit(item, profile) {
  ...
  // A capability named in the title is what the RFP is for; words only in a long description
  // (procurement boilerplate, "financial", "reporting") are weak evidence, so they count for little.
  const titled = capHits.filter((c) => (c.titleHits ?? 0) > 0), described = capHits.filter((c) => !((c.titleHits ?? 0) > 0));
  const capScore = titled.length ? 1 : Math.min(0.4, described.reduce((a, c) => a + ((c.bodyHits ?? 0) >= 3 ? 0.25 : 0.15), 0));
  if (titled.length) reasons.push(`Asks for what you offer: ${titled.map((c) => c.label).join(", ")}`);
  if (described.length) reasons.push(`Mentions what you offer in its description: ${described.map((c) => c.label).join(", ")}`);
  if (!capHits.length && oppCaps.length) reasons.push(`Asks for ${oppCaps.slice(0, 2).map((c) => c.label).join(", ")}, which your profile does not list`);
  ...
  const kwScore = Math.min(1, kwTitle.length * 0.4 + Math.min(0.3, kwText.length * 0.1));
  if (kwTitle.length || kwText.length) reasons.push(`Uses your terms: ${[...kwTitle, ...kwText].slice(0, 5).join(", ")}`);

  // Known platforms by their pattern; one a person typed in, by its name.
  const plats = onOnly(profile.platforms).filter((p) => (PLATFORMS.find(([n]) => n === p.name)?.[1] ?? new RegExp(`(^|[^a-z0-9])${escRe(lower(p.name))}($|[^a-z0-9])`, "i")).test(text));
  const platScore = plats.length ? 1 : 0;
  if (plats.length) reasons.push(`Names ${plats.map((p) => p.name).join(", ")}, which you work with`);

  let score = Math.round(capScore * 60 + kwScore * 25 + platScore * 15);
  if (!titled.length && !kwTitle.length && !plats.length) { score = Math.min(score, 35); reasons.push(capHits.length ? "The title names nothing you offer; only its description mentions it" : "No match to your offering: not relevant unless you know otherwise"); }
  const role = contractorRole(title, text);
  if (role) { score = Math.min(score, 25); reasons.unshift(`Not a solution RFP: ${role}`); }
  const band = score >= 60 ? "strong" : score >= 40 ? "possible" : "weak";
  return { score, band, reasons, contractorRole: role, matched: { capabilities: capHits.map((c) => c.id), terms: [...kwTitle, ...kwText], platforms: plats.map((p) => p.name) }, notOffered: capHits.length ? [] : oppCaps.slice(0, 3).map((c) => c.label) };
}

/**
 * The few search terms that find this company's RFPs on a portal: one per capability it
 * sells (strongest first, e.g. "ERP", "HRIS", "payroll system"), then its platforms
 * ("Business Central"), then its own multi-word terms.
 */
export function liveSearchTerms(profile, max = 6) {
  if (!profile) return [];
  const byId = Object.fromEntries(CAPABILITIES.map((c) => [c.id, c]));
  const terms = [];
  for (const c of onOnly(profile.capabilities).slice().sort((a, b) => (b.strength ?? 0) - (a.strength ?? 0))) { const k = byId[c.id]?.title?.[0]; if (k) terms.push(k); }
  for (const p of onOnly(profile.platforms)) if (!/^(Microsoft 365|Microsoft Azure|AWS|Google Cloud|Copilot|Oracle|SAP)$/.test(p.name)) terms.push(p.name);
  for (const k of (profile.keywords ?? []).filter((k) => k.includes(" "))) terms.push(k);
  return [...new Set(terms.map((t) => String(t).trim()).filter(Boolean))].slice(0, max);
}
...
export default { PLATFORMS, SERVES, pageFacts, textFacts, markdownFacts, profileLinks, buildProfile, profileSummary, companyFit, profileSearch, liveSearchTerms };
