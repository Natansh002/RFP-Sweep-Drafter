...
export function draftAnswers(requirements, knowledge = [], { matrix, now = new Date() } = {}) {
  const entries = knowledge.filter((e) => e && e.question);
  const search = entries.length ? index(entries) : () => [];
  return requirements.map((r) => {
    const hits = search(r.text);
    const sources = hits.map(({ i, s }) => ({ id: entries[i].id, question: entries[i].question, owner: entries[i].owner ?? null, lastReviewed: entries[i].lastReviewed ?? null, stale: isStale(entries[i], now), similarity: Math.round(s * 100) / 100 }));
    const best = hits[0] ? entries[hits[0].i] : null;
    const bestSource = sources[0];
    const usable = best && best.answer && String(best.answer).trim();
    const confidence = !usable ? "low" : bestSource.similarity >= 0.45 && !bestSource.stale ? "high" : bestSource.similarity >= 0.25 ? "medium" : "low";
    const owner = ownerForCategory(r.category, matrix);
    let draft;
    if (usable && confidence !== "low") {
      draft = `${String(best.answer).trim()}\n\n[source: knowledge ${best.id}${best.lastReviewed ? `, reviewed ${best.lastReviewed}` : ""}${bestSource.stale ? ", STALE: confirm before use" : ""}]`;
    } else {
...
export function redTeam(analysis, answers, proposalText = "", { now = new Date() } = {}) {
  ...
  const mandOpen = answers.filter((a) => a.level === "mandatory" && !["Approved", "Final"].includes(a.status));
...
export default { RESPONSE_STATUSES, draftAnswers, buildProposal, redTeam };
