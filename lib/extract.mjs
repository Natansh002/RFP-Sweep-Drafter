...
const FORMATS = {
  ...
  "merx-html"(ch, body) {
    ...
    const pick = (row, cls) => { const m = row.match(new RegExp(`class="${cls}"[^>]*>([\\s\\S]*?)</span>`)); return m ? stripHtml(m[1]) : ""; };
    const date = (row, cls) => { const m = row.match(new RegExp(`class="${cls}"[\\s\\S]{0,1200}?(20\\d\\d)/(\\d\\d)/(\\d\\d)`)); return m ? `${m[1]}-${m[2]}-${m[3]}` : null; };
