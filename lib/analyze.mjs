...
function sentencesWithSections(text) {
  ...
  for (const raw of String(text).split(/\n+/)) {
    ...
    if (h && line.length < 110 && !/[.;:]$/.test(line)) { section = `${h[1].replace(/\.$/, "")} ${h[2]}`.trim(); continue; }
...
export function mapRequirements(text, limit = 300) {
  ...
  const seen = new Set();
  let n = 0;
  for (const { section, text: s } of sentencesWithSections(text)) {
    ...
    seen.add(key);
    n++;
    out.push({ id: `REQ-${String(n).padStart(3, "0")}`, section: section || "—", text: s, level: mand ? "mandatory" : "desirable", category: categorize(s) });
...
export function extractKeyData(text) {
  ...
  const term = find(/\b(?:term|period|duration)\s+of\s+(?:the\s+)?(?:(?:contract|agreement)\s+)?(?:shall\s+be\s+|will\s+be\s+|is\s+)?(?:for\s+)?([a-z]+\s*\(\d+\)|\d+|[a-z]+)\s+(years?|months?)\b/i);
  if (term) {
    const n = Number(term[1].match(/\d+/)?.[0] ?? WORDNUM[term[1].toLowerCase()] ?? NaN);
    if (n) data.contractTerm = `${n} ${term[2].replace(/s?$/, n === 1 ? "" : "s")}`;
  }
  const renew = find(/\b(?:option|options)\s+to\s+(?:renew|extend)[^.]{0,80}/i);
  if (renew) data.renewals = renew[0].trim();
  ...
  // Evaluation criteria: "Technical approach ... 40%" or "Price 30 points".
  const evals = [];
  for (const line of t.split(/\n+/)) {
    const m = line.trim().match(/^(?:[\d.]+\s+|[•\-–*]\s*)?([A-Za-z][A-Za-z ,&/()'-]{2,70}?)\s*[:\-–|.]*\s*(\d{1,3})\s*(%|percent\b|points\b|pts\b)/i);
    if (m && Number(m[2]) <= 100 && !/page|day|year|month/i.test(m[1])) evals.push({ criterion: m[1].trim(), weight: Number(m[2]), unit: /%|percent/i.test(m[3]) ? "%" : "points" });
  }
 ...
 */
export function scoreRfp({ text, requirements, compliance, risks, keyData, packs = [], tenant = null, now = new Date() }) {
  const reasons = [];
  const lower = String(text).toLowerCase();
  const has = (k) => new RegExp(`(^|[^a-z0-9])${k.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}($|[^a-z0-9])`).test(lower);

  // Fit: which industry pack's qualifiers the document hits, plus the tenant's own product words.
  ...
  reasons.push(best.pack
    ? `Fit ${fit}: closest industry is ${best.pack.name}; matched ${best.title.slice(0, 6).join(", ") || "no headline terms"}${best.body.length ? ` and ${best.body.length} supporting term(s)` : ""}${best.disq.length ? `; also mentions ${best.disq.slice(0, 3).join(", ")}` : ""}`
    : "Fit 0: the document matches none of the configured industry packs");
  ...
  return { overall, band, fit, risk, timeline, coverage, weights, industry: best.pack ? { id: best.pack.id, name: best.pack.name } : null, reasons, mandatory: requirements.filter((r) => r.level === "mandatory").length, desirable: requirements.filter((r) => r.level === "desirable").length };
}
...
export function summarize({ title, source, scores, keyData, requirements, compliance, risks }) {
  ...
  const lines = [
    `# ${title || "RFP analysis"}`,
    "",
    `**Overall ${scores.overall}/100: ${scores.band}.** Fit ${scores.fit} · Risk ${scores.risk} · Timeline ${scores.timeline}${scores.coverage == null ? "" : ` · Coverage ${scores.coverage}`}${scores.industry ? ` · closest industry: ${scores.industry.name}` : ""}`,
    ...
    "",
    "## Requirements",
    `- ${requirements.length} found: ${scores.mandatory} mandatory, ${scores.desirable} desirable. Most in: ${topCats || "—"}.`,
...
export function qualify(analysis, meta = {}) {
  ...
  v("Questions deadline", d.questions, "solicitation text");
  v("Pre-bid meeting", d.preBid, "solicitation text");
  v("Contract term", k.contractTerm, "solicitation text");
  v("Stated value", k.value ? `$${Math.round(k.value.amount).toLocaleString("en-US")} (${k.value.context})` : meta.estimatedValue, "solicitation text");
  v("Submission", k.submission?.join(", "), "solicitation text");
  v("Source", meta.url ?? a.source, "link");
  inferred.push({ label: "Fit score", value: `${sc.fit}/100`, how: "keyword match against the industry packs" });
  if (sc.industry) inferred.push({ label: "Closest industry", value: sc.industry.name, how: "best-matching pack" });
  ...
  const matters = [];
  if (sc.fit >= 50 && sc.industry) matters.push(sc.industry.id === "any" ? `Strong match to our capability terms (fit ${sc.fit}).` : `Strong match to ${sc.industry.name} (fit ${sc.fit}).`);
  ...
  if (sc.fit < 35) mayNot.unshift("Weak fit: the document matches few of our industry or capability terms.");
  ...
  if (!d.questions) infoRequired.push("Deadline for questions");
  if (!k.value && !meta.estimatedValue) infoRequired.push("Budget or estimated value");
  if (!k.contractTerm) infoRequired.push("Contract term");
  if (!k.evaluation?.length) infoRequired.push("Evaluation criteria and weights");
  if (!k.submission) infoRequired.push("Submission method and format");
  if ((a.words ?? 0) < 400) infoRequired.push("The full solicitation document. Only a notice summary was analyzed; upload the RFP for a complete read.");

  const high = (a.risks ?? []).filter((r) => r.severity === "high");
  const next = sc.band === "closed" ? "Closed. Record it and move on."
    : (a.words ?? 0) < 400 ? "Get the full RFP document from the source link, then run Analyze RFP on it."
  ...
  return { matters, mayNotQualify: mayNot.length ? mayNot : ["Nothing obvious. Confirm mandatory requirements against the full document."], keyRisks: (a.risks ?? []).slice(0, 6), infoRequired, nextAction: next, verified, inferred };
}

// ------------------------------------------------------------------ all five

export function analyzeRfp({ text, title = "", source = "", proposalText = "", packs = [], tenant = null, now = new Date(), closeDate = null }) {
  ...
  const risks = flagRisks(text, keyData.dates, now);
  const scores = scoreRfp({ text, requirements, compliance, risks, keyData, packs, tenant, now });
