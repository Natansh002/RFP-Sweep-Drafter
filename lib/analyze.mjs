...
export function qualify(analysis, meta = {}) {
  ...
  const looked = meta.rfp ? (docsRead.length ? ` Not stated in the notice or ${docsRead.length === 1 ? `the RFP document read (${docsRead[0]})` : `the ${docsRead.length} RFP documents read`}.` : " Not stated in the notice; no public RFP document could be read.") : "";
  const infoRequired = [];
  if (!d.closing && !meta.closeDate) infoRequired.push(`Closing date and time.${looked}`);
  if (!d.questions) infoRequired.push(`Deadline for questions.${looked}${meta.rfp ? " Ask the contact named in the notice, or check the addenda." : ""}`);
  if (!k.value && !meta.estimatedValue) infoRequired.push(`Budget or estimated value.${looked}${meta.rfp ? " Many buyers do not publish one." : ""}`);
  if (!k.contractTerm) infoRequired.push(`Contract term.${looked}`);
  if (!k.evaluation?.length && !k.evaluationBasis) infoRequired.push(`Evaluation criteria and weights.${looked}`);
  if (!k.submission) infoRequired.push(`Submission method and format.${looked}`);
