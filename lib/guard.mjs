 ...
 *   4. the Claude Code agent definition grants no connector tools for these systems. The one
 *      exception is the sales platform through MCP (scripts/mcp-server.mjs): a person confirms
 *      every record, and the links it records stay in store/crm.json, never in the ledger or
 *      on a published page. The sweep itself still never fetches these domains.
 *
