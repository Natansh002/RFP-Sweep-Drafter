...
export const CAPABILITIES = [
  { id: "erp", label: "ERP / Finance implementation", practice: "Finance Practice",
    ...
    body: ["end of life", "end-of-life", "consolidated reporting", "multi-entity", "intercompany", "chart of accounts", "accounts payable", "procure to pay", "fixed assets", "grant accounting", "fund accounting", "financial reporting", "financial management", "budgeting", "grant management", "expense management"] },
...
export function matchCapabilities(title, text = "") {
  ...
  return CAPABILITIES.map((c) => {
    ...
    return { id: c.id, label: c.label, score: th.length * 3 + new Set(bh).size, titleHits: th.length, bodyHits: new Set(bh).size, matched: [...new Set([...th, ...bh])].slice(0, 8) };
