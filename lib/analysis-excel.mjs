...
export async function buildAnalysisWorkbook(ExcelJS, a, extra = {}) {
  const { answers = null, redTeam = null, team = null, proposal = null, proofread = null } = extra;
  ...
  if (team) {
    ...
    const lab = { rfpManager: "RFP Manager", presales: "Pre-sales Consultant", accountExecutive: "Account Executive", sme: "SME Contributor" };
    for (const [k, l] of Object.entries(lab)) tm.addRow({ k: l, v: `${team[k] ?? l}${k === "sme" && team.smeAreas?.length ? ` (${team.smeAreas.join(", ")})` : ""}` });
    if (team.why) tm.addRow({ k: "Why", v: team.why });
  }
  // Proofread step
  if (proofread) {
    const pr = sheet("Proofread", [{ header: "Severity", key: "s", width: 10 }, { header: "Where", key: "w", width: 16 }, { header: "Issue", key: "k", width: 16 }, { header: "Text", key: "t", width: 60 }, { header: "What to do", key: "d", width: 60 }]);
    pr.addRow({ s: proofread.verdict, w: proofread.signedOff ? `signed off ${String(proofread.signedOff.at).slice(0, 10)}` : "not signed off", k: `${proofread.counts.high} high`, t: `${proofread.counts.medium} medium, ${proofread.counts.low} low`, d: proofread.note }).font = { bold: true };
    for (const i of proofread.issues) wrap(pr.addRow({ s: i.severity, w: i.where, k: i.kind, t: i.text, d: i.suggestion })).getCell("s").fill = fill(i.severity === "high" ? "FFFEE2E2" : i.severity === "medium" ? "FFFEF3C7" : "FFF3F4F6");
  }
  // RFP submission status step
  if (redTeam) {
    const rt = sheet("Submission status", [{ header: "Type", key: "t", width: 12 }, { header: "Item", key: "v", width: 110 }]);
    rt.addRow({ t: "Status", v: redTeam.readiness === "Ready" ? "Ready to submit" : `Not ready: ${redTeam.blocking.length} blocking issue(s)` }).font = { bold: true };
